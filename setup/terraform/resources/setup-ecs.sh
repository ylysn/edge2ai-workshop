...
EOF
  ...
  # TODO: Fix canal from vxlan to host-gw
...
if [[ ! -z ${CLUSTER_ID:-} ]]; then
  ...
  figlet -f small -w 300  "ECS  ${CLUSTER_ID:-???}  deployed successfully"'!' | cowsay -n -f "$(ls -1 $COWPATH | grep "\.cow" | sed 's/\.cow//' | egrep -v "bong|head-in|sodomized|telebears" | shuf -n 1)"
